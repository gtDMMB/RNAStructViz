...
RNAStructure* RNAStructure::CreateFromFile(const char* filename, const bool isBPSEQ)
{
	...
	switch (base)
	{
		...
		if (strlen(filename) > 980) 
		    fl_message("Bad base: id %d, <file name too long>", result->m_sequenceLength + 1);
		else
		    fl_message("Bad base: id %d, file %s", result->m_sequenceLength + 1, filename);
		delete result;
	    	inStream.close();
		return 0;
	    }
	...
	if (!isBPSEQ)
	{
	    ...
	    {
		if (strlen(filename) > 980) 
		    fl_message("Bad prev id: id %d, <file name too long>", result->m_sequenceLength + 1);
		else
		    fl_message("Bad prev id: id %d, file %s", result->m_sequenceLength + 1, filename);
		delete result;
		inStream.close();
		return 0;
	    }
	    ...
	    {
		if (strlen(filename) > 980) 
		    fl_message("Bad next id: id %d, <file name too long>", result->m_sequenceLength + 1);
		else
		    fl_message("Bad next id: id %d, file %s", result->m_sequenceLength + 1, filename);
		delete result;
		inStream.close();
		return 0;
	    }
	...
	{
	    if (strlen(filename) > 980) 
		fl_message("Bad pair: id %d, <file name too long>", result->m_sequenceLength + 1);
	    else
		fl_message("Bad pair: id %d, file %s", result->m_sequenceLength + 1, filename);
	    delete result;
	    inStream.close();
	    return 0;
	}
	...
	if (!isBPSEQ)
	{
	    ...
	    {
		if (strlen(filename) > 980) 
		    fl_message("Bad trailing id: id %d, <file name too long>", result->m_sequenceLength + 1);
		else
		    fl_message("Bad trailing id: id %d, file %s", result->m_sequenceLength + 1, filename);
		delete result;
		inStream.close();
		return 0;
	    }
	...
	{
	    maxSize += 1024;
	    result->m_sequence = (BaseData*)realloc(result->m_sequence, sizeof(BaseData) * maxSize);
	}
    }
    ...
    result->m_sequence = (BaseData*)realloc(result->m_sequence, sizeof(BaseData) * result->m_sequenceLength);
...
void RNAStructure::DisplayFileContents()
{
	...
	Fl_Text_Buffer* textBuffer = new Fl_Text_Buffer(strlen(m_displayString + 1));
...
void RNAStructure::GenerateString()
{
    ...
    */
    ...
    m_displayString = (char*)malloc(sizeof(char) * size);
    ...
    m_displayString = (char*)realloc(m_displayString, sizeof(char) * (size - remainingSize));
}
